from utils import getCollectionPhotos, \
                  getPoints, \
        ...
        if len(points) != M * N:
            ...
            if DO_SHADOW:
                for p in shadowImagePoints:
                    cv2.circle(imgShadow, (int(p[0][0]), int(p[0][1])), 1, (0, 0, 0), -1)
            ...
            for p_ind in range(objectImagePoints.shape[0]):
                ...
                cv2.circle(imgShape, (int(p[0][0]), int(p[0][1])), 1, (p_color, p_color, p_color), -1)
