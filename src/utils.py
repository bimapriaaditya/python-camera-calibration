...
def get_save_address(COLLECTION_NUM, IMAGE_NUM, DO_COLOR, SMOOTHING):
    collection_result_folder = os.path.join(RESULTS_FOLDER, str(COLLECTION_NUM))
    if not os.path.exists(collection_result_folder):
        os.makedirs(collection_result_folder)
    
    name = "%d-color:%s-smoothing:%s%s" % (IMAGE_NUM, str(DO_COLOR), str(SMOOTHING), IMG_FILE_EXTENSION)

    return os.path.join(collection_result_folder, name)
            ...
            if mode == 'h':
                ...
                if len(parts) == 1:
                    if parts[0].strip() == "end_header":
                        ...
                        faces = [[] for x in range(vertex_len)]
                        continue
...
def getPoints(img, COLLECTION_NUM, IMAGE_NUM, M, N):
    ...
    def draw_points(imgO, radius=4, thickness=2):
        ...
        i = 0
        for point in points:
            cv2.circle(imgD, (int(point[0]), int(point[1])), radius, COLOR[int((i/M) % len(COLOR))], thickness)
            i += 1
        return imgD
...
def getCollectionPhotos(collectionNum, scale_down_factor=1):
    ...
    for f in files:
        cimg = cv2.imread(f)
        h, w, _ = cimg.shape
        cimg = cv2.resize(cimg, (int(w/scale_down_factor), int(h/scale_down_factor)))
        imgs.append(cimg)
...
def draw3DAxisLines(img, origin, imgPoints):
    corner = tuple(map(int, origin.ravel()))
    cv2.line(img, corner, tuple(map(int, imgPoints[0].ravel())), (255, 0, 0), 5)
    cv2.line(img, corner, tuple(map(int, imgPoints[1].ravel())), (0, 255, 0), 5)
    cv2.line(img, corner, tuple(map(int, imgPoints[2].ravel())), (0, 0, 255), 5)
    return img
